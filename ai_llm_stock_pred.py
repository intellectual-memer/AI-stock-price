...
def process_sentiment_data(news_df):
    ...
    # Create rolling averages that count number of positive and negative sentiment articles within t days
    grouped['7_day_avg_positive'] = grouped['POSITIVE'].rolling(window=7, min_periods=1).sum()
    grouped['7_day_avg_negative'] = grouped['NEGATIVE'].rolling(window=7, min_periods=1).sum()
...
def get_stock_data(ticker, start_date, end_date):
    ...
    stock_data['Pct_Change'] = stock_data['Close'].pct_change() * 100 # Transform closing values to percent change
    return stock_data

# Function to combine sentiment and stock data
def combine_data(result_df, stock_date):
    combined_df = result_df.set_index('DateOnly').join(stock_data[['Pct_Change']], how='inner')
    combined_df['lagged_7day_pct_positive'] = combined_df['7day_pct_positive'].shift(1) # Lag sentiment features
    return combined_df
...
def fit_and_forecast(combined_df, forecast_steps=3):
    ...
    future_exog = combined_df['lagged_7day_pct_positive'][-forecast_steps:].values.reshape(-1,1) # Future ex
...
def create_plot(combined_df, forecast_mean, forecast_ci, forecast_index):
    # Standardize the sentimental proportion
